...
test('Circle class area method', () => {
    ...
    expect(circle.render(shapeColor)).toBe('<circle cx="150" cy="100" r="80" fill="#ff0000" />');
});

test('Rectangle class area method', () => {
    const rectangle = new Rectangle(10, 20);
    const shapeColor = "#00ff00"; // Mock shapeColor for testing
    expect(rectangle.render(shapeColor)).toBe('<rect x="70" y="20" width="160" height="160" fill="#00ff00" />');
});

test('Triangle class area method', () => {
    const triangle = new Triangle();
    const shapeColor = "#0000ff"; // Mock shapeColor for testing
    expect(triangle.render(shapeColor)).toBe('<polygon points="150,10 290,190 10,190" fill="#0000ff" />');
});